...
public class SortNaviBar extends RelativeLayout implements OnClickListener{
	...
	//选择的文本颜色
	private int selTextColor = 0xFFFF2640;

	public SortNaviBar(Context context, AttributeSet attrs, int defStyleAttr) {
		super(context, attrs, defStyleAttr);
		// TODO Auto-generated constructor stub
		init(context);
	}

	public SortNaviBar(Context context, AttributeSet attrs) {
		super(context, attrs);
		// TODO Auto-generated constructor stub
		init(context);
	}

	public SortNaviBar(Context context) {
		super(context);
		// TODO Auto-generated constructor stub
		init(context);
	}
	...
	public void changeNavi(int postion,boolean isNotifyListner) {
		if (postion != naviPostion) {
			...
			// 改变之前箭头方向样式
			if ((naviPostion == 1) || (naviPostion == 2))// 修改图标颜色
			...
			// 高低排序
			arrowDirectionNavi01 = (arrowDirectionNavi01 + 1) % 2;
			//改变箭头方向
			changeArrow(postion, true);
			//通知变化
			if(isNotifyListner && onNaviBarChangeListener != null)
				onNaviBarChangeListener.onArrowChanged(postion,arrowDirectionNavi01);
		}else if(postion == 2){
			// 高低排序
			arrowDirectionNavi02 = (arrowDirectionNavi02 + 1) % 2;
			//改变箭头方向
			changeArrow(postion, true);
			//通知变化
			if(isNotifyListner && onNaviBarChangeListener != null)
				onNaviBarChangeListener.onArrowChanged(postion,arrowDirectionNavi02);
		}
	}
	...
	private void changeArrow(int postion, boolean isSel) {
		...
		if(isSel)
			drawableRight = CommonUtils.GetDrawable(context,getArrowDirection() == 0 ? R.drawable.mmh_ic_sortdown_checked : R.drawable.mmh_ic_sortup_checked);
		else
			drawableRight = CommonUtils.GetDrawable(context,getArrowDirection() == 0 ? R.drawable.mmh_ic_sortdown_normal: R.drawable.mmh_ic_sortup_normal);
	...
	public int getArrowDirection() {
		return naviPostion == 1 ? arrowDirectionNavi01 : arrowDirectionNavi02;
	}
